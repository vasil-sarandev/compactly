...
import { shortenedUrlRepository } from '@/components/shortened-url/shortened-url.repository';
...
class SlugTopicsConsumer {
  ...
  handleSlugPoolLowCountTransaction = async ({ type }: SlugPoolLowCountTopicMessage) => {
...
export const slugTopicsConsumer = new SlugTopicsConsumer();
